...
- Self-update like Cursor: `claude-desktop --update` keeps the locally-built package current with upstream Claude Desktop releases, and `claude-desktop --setup-auto-update` installs a daily systemd user timer that runs it silently. The newest packaged Claude version is read from upstream's git tags (`vREPO+claudeX.Y.Z`) rather than Anthropic's Cloudflare-gated download endpoint, so the check is a cheap API call — no Playwright/Chromium. When behind, the upstream release tag is merged into a throwaway git worktree (the user's checkout is never touched) so the build gets both the new download URLs *and* upstream's updated patch scripts while keeping this fork's commits; the resulting `.deb` is installed via a tiny root-owned wrapper (`/usr/local/sbin/claude-desktop-apply-update`) whitelisted by a scoped NOPASSWD sudoers drop-in, which validates the package is `claude-desktop` before `dpkg -i`. A failed or patch-incompatible build (caught by `verify-patches`) aborts before install, so the working app is never replaced by a broken one; merge conflicts bail with a "manual merge needed" notification. `--update --check` (exit 10 if behind, for the timer) and `--update --dry-run` are non-destructive. A `merge=union` driver on `CHANGELOG.md` (`.gitattributes`) keeps the unattended merges conflict-free. Available in all four launchers (deb, RPM, AppImage, Nix). Logic covered by `tests/auto-update.bats`.
- BATS coverage for the deb maintainer scriptlets: the postinst/postrm bodies moved from inline heredocs in `scripts/packaging/deb.sh` to generator functions in `scripts/packaging/deb-scriptlets.sh` (same logic, paths now parameterized for tests; production output is unchanged apart from quoting). The new `tests/deb-scriptlets.bats` exercises the previously untestable install/remove logic as a regular user against a temp prefix: chrome-sandbox 4755 handling, the userns kernel-knob gate, AppArmor profile write/load, marker-based preservation of hand-edited profiles, deferral to foreign bwrap profiles, parser-rejection cleanup, idempotency, and postrm's remove/purge/upgrade semantics (unload on remove, delete on purge only, no-op on upgrade).
- Cowork KVM setup command: `claude-desktop --cowork-setup` checks everything the KVM micro-VM backend needs (CPU virtualization, `/dev/kvm` access, QEMU, socat, virtiofsd, the `vhost_vsock` module) and prints the exact per-distro commands (apt/dnf/pacman) to close each gap; `--cowork-setup --install` runs them via sudo. Doctor already diagnosed these as missing but left the install work to the user. Handles the distro quirks: virtiofsd installed off-PATH gets a symlink instead of a package, Arch's virtiofsd ships inside `qemu-full` (deduplicated when QEMU is also planned), `/dev/kvm` permission gaps plan a `kvm` group membership with a re-login note, and the vsock module is both loaded and persisted via `/etc/modules-load.d/`. Dry-run by default — without `--install` nothing is executed. Available in all four launchers (deb, RPM, AppImage, Nix).
...
- The deb and RPM packages now declare their runtime dependencies (case-docs S03/S04): the bundled Electron links the canonical Chromium/GTK system set (`ldd` shows libgtk-3, libnss3, libatspi, libasound as direct links), so the control file declares `Depends: libgtk-3-0, libnotify4, libnss3, libxss1, libxtst6, xdg-utils, libatspi2.0-0, libuuid1, libsecret-1-0, libasound2` and the RPM spec a matching manual `Requires:` block (gtk3, libnotify, nss, libXScrnSaver, libXtst, xdg-utils, at-spi2-core, libuuid, libsecret, alsa-lib — `AutoReqProv: no` stays, since rpm's scanner emits unsatisfiable junk from the bundled payload). The RPM also gains `Recommends: bubblewrap` for parity with the deb. Desktop installs already have these libraries; the declarations fix first-launch failures in minimal containers/VMs. The S03/S04 harness specs flip from `test.fail()` regression detectors to real guards, and the artifact tests assert the declarations.
